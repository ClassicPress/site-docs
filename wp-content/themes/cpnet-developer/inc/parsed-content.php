...
class WPORG_Edit_Parsed_Content {
	...
	public function parsed_meta_box_cb( $post ) {
		...
		if ( $ticket ) {
			$src  = "https://github.com/ClassicPress/ClassicPress/issues/{$ticket}";
			$ticket_message = sprintf( '<a href="%1$s">%2$s</a>', esc_url( $src ), apply_filters( 'the_title', $ticket_label ) );
		} else {
			$link = sprintf( '<a href="https://github.com/ClassicPress/ClassicPress/issues/new">%s</a>', __( 'ClassicPress GitHub', 'wporg' ) );
		...
		<table class="form-table">
			...
			<?php if ( current_user_can( 'manage_options' ) ) : ?>
				<tr valign="top" id="ticket_controls">
					<th scope="row">
						<label for="wporg_parsed_ticket"><?php _e( 'ClassicPress GitHub Ticket Number:', 'wporg' ); ?></label>
					</th>
					<td>
						<span class="attachment_controls">
							...
							<a href="#attach-ticket" class="button secondary <?php echo $ticket ? 'hidden' : ''; ?>" id="wporg_ticket_attach" name="wporg_ticket_attach" aria-label="<?php esc_attr_e( 'Attach a ClassicPress GitHub ticket', 'wporg' ); ?>" data-nonce="<?php echo wp_create_nonce( 'wporg-attach-ticket' ); ?>" data-id="<?php the_ID(); ?>">
								<?php esc_attr_e( 'Attach Ticket', 'wporg' ); ?>
							</a>
							<a href="#detach-ticket" class="button secondary <?php echo $ticket ? '' : 'hidden'; ?>" id="wporg_ticket_detach" name="wporg_ticket_detach" aria-label="<?php esc_attr_e( 'Detach the ticket', 'wporg' ); ?>" data-nonce="<?php echo wp_create_nonce( 'wporg-detach-ticket' ); ?>" data-id="<?php the_ID(); ?>">
	...
	public function attach_ticket() {
		...
		$ticket_url = "https://github.com/ClassicPress/ClassicPress/issues/{$ticket_no}";
		...
		if ( 200 === $status_code && null !== $body ) {
			...
			if ( class_exists( 'DOMDocument' ) ) {
				...
				$title = str_ireplace( ' – ClassicPress GiHub', '', $title );
			} else {
				die( -1 );
			}
