...
class DevHub_Head {
	...
	public static function output_head_tags() {
		...
		if ( is_front_page() ) {
			$desc = __( 'Official ClassicPress developer resources including a code reference, handbooks (for APIs, plugin and theme development, block editor), and more.', 'wporg' );
		}
		elseif ( is_page( 'reference' ) ) {
			$desc = __( 'Want to know what&#8217;s going on inside ClassicPress? Find out more information about its functions, classes, methods, and hooks.', 'wporg' );
		}
		elseif ( DevHub\is_parsed_post_type() ) {
			...
			elseif ( is_post_type_archive() ) {
				...
				$desc = sprintf( __( 'Code Reference archive for ClassicPress %s.', 'wporg' ), strtolower( $post_type_items ) );
			}
		}
		elseif ( is_singular() ) {
