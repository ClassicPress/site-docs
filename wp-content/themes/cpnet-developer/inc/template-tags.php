			...
			foreach ( $comments as $comment ) {
				...
				if ( $can_user_post_note ) {
					...
					if ( ! $is_user_logged_in ) {
						...
						$feedback_link = 'https://docs.classicpress.net/wp-login.php/?redirect_to=' . urlencode( $feedback_link );
					} else {
						$class         ='add';
...
namespace DevHub {
	 ...
	 * WP_CORE_LATEST_RELEASE constant (set on .org) though this is not
	...
	function get_source_file_link( $post_id = null, $line_number = true ) {
		...
		if ( ! empty( $source_file ) ) {
			$url = 'https://github.com/ClassicPress/ClassicPress/blob/master/src/' . $source_file;
			// Line number.
	...
	function get_reference_template_parts() {
		...
		$templates = array(
			...
			'methods',
			'changelog',
			//'notes' Enable this for Comments.
		);
